...
// lib/plugins/gamecubate/automata/automaton.js
ig.baked=true;ig.module('plugins.gamecubate.automata.automaton').defines(function(){CellState={DEAD:0,ALIVE:1};Automaton=ig.Class.extend({cols:0,rows:0,inputData:null,data:null,init:function(cols,rows,data)
{this.inputData=data;this.cols=cols;this.rows=rows;this.reset();},reset:function()
{if(this.inputData)
{this.data=this.inputData;return;}
else
{this.data=[];for(var row=0;row<this.rows;row++)
{this.data[row]=[];for(var col=0;col<this.cols;col++)
this.data[row][col]=CellState.DEAD;}}},populate:function(livingRatio,padding)
{var margin=padding|0;for(var row=margin;row<this.rows-margin;row++)
{for(var col=margin;col<this.cols-margin;col++)
{if(Math.random()<livingRatio)
this.data[row][col]=CellState.ALIVE;}}},stateAt:function(col,row)
{if(this.data[row])
return this.data[row][col];},neighborsOf:function(col,row)
{var neighbors=[];var state;state=this.stateAt(col-1,row-1);if(state!=undefined)neighbors[neighbors.length]=state;state=this.stateAt(col,row-1);if(state!=undefined)neighbors[neighbors.length]=state;state=this.stateAt(col+1,row-1);if(state!=undefined)neighbors[neighbors.length]=state;state=this.stateAt(col-1,row);if(state!=undefined)neighbors[neighbors.length]=state;state=this.stateAt(col+1,row);if(state!=undefined)neighbors[neighbors.length]=state;state=this.stateAt(col-1,row+1);if(state!=undefined)neighbors[neighbors.length]=state;state=this.stateAt(col,row+1);if(state!=undefined)neighbors[neighbors.length]=state;state=this.stateAt(col+1,row+1);if(state!=undefined)neighbors[neighbors.length]=state;return neighbors;},foundWithState:function(state,states)
{var found=0;for(var i=0;i<states.length;i++)
{var theState=states[i];if(theState==state)
{found+=1;}}
return found;},step:function()
{},});});

// lib/plugins/gamecubate/automata/conway.js
ig.baked=true;ig.module('plugins.gamecubate.automata.conway').requires('plugins.gamecubate.automata.automaton').defines(function(){Conway=Automaton.extend({step:function()
{var nextStates=ig.copy(this.data);for(var row=0;row<this.rows;row++)
{for(var col=0;col<this.cols;col++)
{var state=this.stateAt(col,row);var neighbors=this.neighborsOf(col,row);var liveNeighbors=this.foundWithState(CellState.ALIVE,neighbors);if(state==CellState.ALIVE&&liveNeighbors<2)
...
{nextStates[row][col]=CellState.ALIVE;}}}
this.data=ig.copy(nextStates);return this.data;}});});

// lib/plugins/symbols/symbols.js
ig.baked=true;ig.module('plugins.symbols.symbols').requires('impact.impact').defines(function(){ig.Symbols=ig.Class.extend({instance:null,staticInstantiate:function(ignore){if(ig.Symbols.instance==null){return null;}
else{throw("Error: ig.Symbols has already been instantiated. It can only be instantiated once.");return ig.Symbols.instance;}},init:function(names,classes){if(typeof classes=="undefined")
classes=[ig.Entity];var nameArray=names.split(" "),nameArrayLen=nameArray.length;if(nameArrayLen>30){throw("Error: ig.Symbols allows a maximum of 30 symbol definitions.");}
for(var i=0;i<classes.length;i++){var currentClass=classes[i];for(var j=0;j<nameArrayLen;j++){var name=nameArray[j];name.replace(/^\s+|\s+$/g,"");if(name){currentClass[name]=0x1<<j;}}}
ig.Symbols.instance=this;}});});

// lib/plugins/outlinedfont.js
ig.baked=true;ig.module('plugins.outlinedfont').requires('impact.font').defines(function(){OutlinedFont=ig.Font.extend({outlineWidth:1,init:function(path,outlineWidth){this.outlineWidth=outlineWidth;this.parent(path);},draw:function(text,x,y,align){if(typeof(text)!='string'){text=text.toString();}
if(text.indexOf('\n')!==-1){var lines=text.split('\n');for(i=0;i<lines.length;i++){this.draw(lines[i],x,y+i*(this.height-this.outlineWidth),align);}
return;}
if(align==ig.Font.ALIGN.RIGHT||align==ig.Font.ALIGN.CENTER){if(align==ig.Font.ALIGN.RIGHT){x+=this.outlineWidth;}
var width=(2*this.outlineWidth);for(var i=0;i<text.length;i++){var c=text.charCodeAt(i);width+=this.widthMap[c-this.firstChar]+1-(2*this.outlineWidth);}
x-=align==ig.Font.ALIGN.CENTER?width/2:width;}
if(align==ig.Font.ALIGN.LEFT){x-=this.outlineWidth;}
for(var i=0;i<text.length;i++){var c=text.charCodeAt(i);x+=this._drawChar(c-this.firstChar,x,y);}
ig.Image.drawCount+=text.length;},_drawChar:function(c,targetX,targetY){if(!this.loaded||c<0||c>=this.indices.length){return 0;}
var scale=ig.system.scale;var charX=this.indices[c]*scale;var charY=0;var charWidth=this.widthMap[c]*scale;var charHeight=(this.height-2)*scale;ig.system.context.drawImage(this.data,charX,charY,charWidth,charHeight,ig.system.getDrawPos(targetX),ig.system.getDrawPos(targetY),charWidth,charHeight);return this.widthMap[c]+1-(2*this.outlineWidth);}});});
...
// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.background-map','plugins.gamecubate.automata.conway','plugins.symbols.symbols','plugins.outlinedfont','plugins.impact-splash-loader').defines(function(){PluginDemo=ig.Game.extend({clearColor:null,gameState:null,COLS:40,ROWS:60,ca1:null,ca2:null,ca3:null,ca4:null,overlay:null,simulationTimer:null,stepDuration:0.1,font:new OutlinedFont('media/outlinedfont.png',1),init:function()
{new ig.Symbols("PLAYING PAUSED");this.initAutomata();this.initMap();this.simulationTimer=new ig.Timer();ig.input.bind(ig.KEY.SPACE,'pause');ig.input.bind(ig.KEY.RIGHT_ARROW,'step');ig.input.bind(ig.KEY.LEFT_ARROW,'reset');ig.input.bind(ig.KEY.MOUSE1,'mouse1');this.gameState=ig.Entity.PLAYING;},initMap:function()
{var data=this.createData(this.COLS,this.ROWS,2);var bg=new ig.BackgroundMap(8,data,new ig.Image('media/tiles.png'));bg.preRender=true;this.backgroundMaps.push(bg);data=this.createData(this.COLS,this.ROWS,0);this.overlay=new ig.BackgroundMap(8,data,new ig.Image('media/tiles.png'));this.backgroundMaps.push(this.overlay);},resetMap:function()
{this.overlay.data=this.createData(this.COLS,this.ROWS,0);},initAutomata:function()
{this.ca1=new Conway(10,60);this.ca1.populate(0.4);this.ca2=new Conway(10,60);this.ca2.populate(0.4);this.ca3=new Conway(10,60);this.ca3.populate(0.4);this.ca4=new Conway(10,60);this.ca4.populate(0.4);},resetAutomata:function()
{this.ca1.reset();this.ca1.populate(0.4);this.ca2.reset();this.ca2.populate(0.4);this.ca3.reset();this.ca3.populate(0.4);this.ca4.reset();this.ca4.populate(0.4);},createData:function(cols,rows,fillValue)
{var fill=fillValue|0;var data=[];for(var row=0;row<rows;row++)
{data[row]=[];for(var col=0;col<cols;col++)
data[row][col]=fill;}
return data;},update:function()
{if(this.gameState&ig.Entity.PLAYING&&(this.simulationTimer.delta()>0&&this.stepDuration>0))
{this.step();this.simulationTimer.set(this.stepDuration);}
this.handleKeys();this.handleMouse();this.parent();},step:function()
{this.ca1.step();this.ca2.step();this.ca3.step();this.ca4.step();this.updateMap(this.ca1,this.overlay,1,0,0);this.updateMap(this.ca2,this.overlay,4,10,0);this.updateMap(this.ca3,this.overlay,3,20,0);this.updateMap(this.ca4,this.overlay,6,30,0);},reset:function()
{this.resetAutomata();this.resetMap();this.updateMap(this.ca1,this.overlay,1,0,0);this.updateMap(this.ca2,this.overlay,4,10,0);this.updateMap(this.ca3,this.overlay,3,20,0);this.updateMap(this.ca4,this.overlay,6,30,0);},updateMap:function(automaton,map,tileIndex,x,y)
{var states=automaton.data;for(var row=0;row<automaton.rows;row++)
{for(var col=0;col<automaton.cols;col++)
{var state=states[row][col];if(state==CellState.DEAD)
{map.data[row+y][col+x]=0;}
else if(state==CellState.ALIVE)
{map.data[row+y][col+x]=tileIndex;}}}},handleKeys:function()
...
{this.reset();}},handleMouse:function()
{if(ig.input.released("mouse1"))
{this.reset();}},draw:function()
{ig.system.context.clearRect(0,0,ig.system.realWidth,ig.system.realHeight);this.parent();this.font.draw('SPACE: PAUSE/PLAY     LEFT, MOUSE: RESET     RIGHT: STEP',ig.system.width/2,5,ig.Font.ALIGN.CENTER);},});ig.main('#canvas',PluginDemo,30,320,480,1,ig.ImpactSplashLoader);});