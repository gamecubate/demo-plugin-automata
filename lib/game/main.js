...
Demo = ig.Game.extend({
	...
	// Controls the interval at which we tell the automata to advance by one step
	...
	{
		map.data = this.createData (this.COLS, this.ROWS, 0);
	},

	initAutomata: function ()
	{
		// right shifter
		this.ca1 = new Shifter (10,60);
		this.ca1.directionFunction = function(row,col)
		{
			return {hdir:1, vdir:0};
		};
		this.ca1.populate (0.4);


		// random shifter
		this.ca2 = new Shifter (10,60);
		/*this.ca2.directionFunction = function(row,col)
		{
			return {hdir:(row+col)%2==0?1:-1, vdir:0};
		};*/
		this.ca2.directionFunction = function (row, col)
		{
			return {hdir:(Math.random() < 0.5) ? 1 :-1, vdir:(Math.random() < 0.5) ? 1 : -1};
		}
		this.ca2.populate (0.4);


		// Conways's Game of Life automaton
		this.ca3 = new Conway (10,60);
		this.ca3.populate (0.4);


		// down shifter
		this.ca4 = new Shifter (10,60);
		this.ca4.directionFunction = function(row,col)
		{
			return {hdir:0, vdir:1};
		};
...
ig.main ('#canvas', Demo, 30, 320, 480, 1, ig.ImpactSplashLoader);
